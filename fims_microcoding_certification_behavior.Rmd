...
create_kappa_baserates_dfs <- function(merged_dfs) {
  ...
  for (i in 1:length(merged_dfs)) {
    ...
    merged_dfs[[names(merged_dfs)[i]]] <- mutate(merged_dfs[[names(merged_dfs)[i]]],
      ...
      active_consens = ifelse(str_detect(`Consensus Code`, "active"), 1, 0),
      across(contains("_code") & !contains("agree"), ~ifelse(str_detect(., "active"), 1, 0), .names="active_{col}"),
      ...
      withdraw_consens = ifelse(str_detect(`Consensus Code`, "withdraw"), 1, 0),
      across(contains("_code") & !contains("agree"), ~ifelse(str_detect(., "withdraw"), 1, 0), .names="withdraw_{col}"),
      offtask_consens = ifelse(str_detect(`Consensus Code`, "off"), 1, 0),
      across(contains("_code") & !contains("agree"), ~ifelse(str_detect(., "off"), 1, 0), .names="offtask_{col}"),
      nonauto_consens = ifelse(str_detect(`Consensus Code`, "auto"), 1, 0),
      across(contains("_code") & !contains("agree"), ~ifelse(str_detect(., "auto"), 1, 0), .names="nonauto_{col}"),
      reject_consens = ifelse(str_detect(`Consensus Code`, "reject"), 1, 0),
      across(contains("_code") & !contains("agree"), ~ifelse(str_detect(., "reject"), 1, 0), .names="reject_{col}")
      )
...
create_kappa_allcoder_dfs <- function(merged_dfs) {
  ...
  for (i in 1:length(merged_dfs)) {
    ...
    kappas_allcoders_df <- mutate(kappas_allcoders_df,
          neutral_code = ifelse(str_detect(code, regex("neutral", ignore.case=TRUE)), 1, 0),
          neutral_consens = ifelse(str_detect(`Consensus Code`, regex("neutral", ignore.case=TRUE)), 1, 0),
          active_code = ifelse(str_detect(code, regex("active", ignore.case=TRUE)), 1, 0),
          active_consens = ifelse(str_detect(`Consensus Code`, regex("active", ignore.case=TRUE)), 1, 0),
          positive_consens = ifelse(str_detect(`Consensus Code`,regex("positive", ignore.case=TRUE)), 1, 0),
          positive_code = ifelse(str_detect(code, regex("positive", ignore.case=TRUE)), 1, 0),
          uncodable_consens = ifelse(str_detect(`Consensus Code`, regex("uncodable", ignore.case=TRUE)), 1, 0),
          uncodable_code = ifelse(str_detect(code, regex("uncodable", ignore.case=TRUE)), 1, 0),
          withdraw_consens = ifelse(str_detect(`Consensus Code`, regex("withdraw", ignore.case=TRUE)), 1, 0),
          withdraw_code = ifelse(str_detect(code, regex("withdraw", ignore.case=TRUE)), 1, 0),
          offtask_consens = ifelse(str_detect(`Consensus Code`, regex("off", ignore.case=TRUE)), 1, 0),
          offtask_code = ifelse(str_detect(code, regex("off", ignore.case=TRUE)), 1, 0),
          nonauto_consens = ifelse(str_detect(`Consensus Code`, regex("auto", ignore.case=TRUE)), 1, 0),
          nonauto_code = ifelse(str_detect(code, regex("auto", ignore.case=TRUE)), 1, 0),
          reject_consens = ifelse(str_detect(`Consensus Code`, regex("reject", ignore.case=TRUE)), 1, 0),
          reject_code = ifelse(str_detect(code, regex("reject", ignore.case=TRUE)), 1, 0)
          )
...
mega_df <- mutate(mega_df,
      ...
      across(ends_with("_code") & !contains("agree"), ~ifelse(grepl("active", ., ignore.case=TRUE), 1, 0), .names="positive_{sub('_code', '', col)}"),
      #positive
      ...
      across(ends_with("_code") & !contains("agree"), ~ifelse(grepl("withdraw", ., ignore.case=TRUE), 1, 0), .names="withdraw_{sub('_code', '', col)}"),
      #offtask
      offtask_consens = ifelse(grepl("off", `Consensus Code`, ignore.case=TRUE), 1, 0),
      across(ends_with("_code") & !contains("agree"), ~ifelse(grepl("off", ., ignore.case=TRUE), 1, 0), .names="offtask_{sub('_code', '', col)}"),
      #nonauto
      ...
      across(ends_with("_code") & !contains("agree"), ~ifelse(grepl("reject", ., ignore.case=TRUE), 1, 0), .names="reject_{sub('_code', '', col)}")
      )
...
# first remember the new column names
names <- c("active", "neutral", "nonauto", "offtask", "positive", "reject", "total", "uncodable", "withdraw") #ordered alphabetically 
# transpose all but the first column (names)
