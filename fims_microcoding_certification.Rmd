...
```{r}
indiv_dfs <- map2(kappa_dfs, perc_agree_dfs, dplyr::full_join, by = "coder") #for each entry of these lists, full join them by the coder column
```
...
lapply(1:length(indiv_dfs), function(i) write.csv(indiv_dfs[[i]], 
                                      file = paste0(str_c(path, sub("_kappas", "", names(indiv_dfs)[i]), "_summary.csv", sep="")), #call participantid_parent/child_summary
                                      row.names = FALSE))
```
