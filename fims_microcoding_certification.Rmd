...
mega_df <- mutate(mega_df,
      ...
      neg_dys_consens = ifelse(grepl("negative - dysphoric", `Consensus Code`) | grepl("negative – dysphoric", `Consensus Code`), 1, 0),
      across(ends_with("_code") & !contains("agree"), ~ifelse(grepl("negative - dysphoric", .) | grepl("negative – dysphoric", .) , 1, 0), .names="neg_dys_{sub('_code', '', col)}"),
      neg_angry_consens = ifelse(grepl("negative - angry", `Consensus Code`) | grepl("negative – angry", `Consensus Code`), 1, 0),
      across(ends_with("_code") & !contains("agree"), ~ifelse(grepl("negative - angry", .) | grepl("negative – angry", .), 1, 0), .names="neg_angry_{sub('_code', '', col)}"),
      )
...
# first remember the new column names
names <- c("end", "neg_angry", "neg_dys", "neutral", "positive", "start", "uncodable")
# transpose all but the first column (names)
...
for (k in 1:length(coders_mega)) {
  for (j in 1:length(codes)){
    if (codes[j] != "total") { #for all codes except total kappa...
      ...
      code_rate_coder = str_c(codes[j], sub('_code', '', coders_mega[k]), sep="_")
