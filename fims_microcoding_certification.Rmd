...
create_kappa_baserates_dfs <- function(merged_dfs) {
  ...
  for (i in 1:length(merged_dfs)) {
    ...
    merged_dfs[[names(merged_dfs)[i]]] <- mutate(merged_dfs[[names(merged_dfs)[i]]],
      ...
      across(contains("_code") & !contains("agree"), ~ifelse(str_detect(., "negative - dysphoric") | str_detect(., "negative – dysphoric"), 1, 0), .names="neg_dys_{col}"),
      neg_angry_consens = ifelse(str_detect(`Consensus Code`, "negative - angry") | str_detect(`Consensus Code`, "negative – angry"), 1, 0),
      across(contains("_code") & !contains("agree"), ~ifelse(str_detect(., "negative - angry") | str_detect(., "negative – angry"), 1, 0), .names="neg_angry_{col}"),
      )
