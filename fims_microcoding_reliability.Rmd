...
#create empty dfs to percent agreements
all_summary_df <- data.frame(matrix(NA, nrow=(length(all_coders)+2), ncol=length(codes))) #+2 rows for kappa_allcoders and percent_agree_allcoders
parent_summary_df <- data.frame(matrix(NA, nrow=(length(parent_coders)+2), ncol=length(codes)))
child_summary_df <- data.frame(matrix(NA, nrow=(length(child_coders)+2), ncol=length(codes)))
...
colnames(all_summary_df)[6] <- paste0(codes[6])
colnames(parent_summary_df)[6] <- paste0(codes[6])
colnames(child_summary_df)[6] <- paste0(codes[6])
...
### Calculate overall percent agreement and kappa for parent_summary and child_summary
```{r}
#get list of coder pairs
coderpairs <- names(coder_dfs)

#extract two coder names from each pair, put in dataframe: coder1, coder2

#total percent agreement: sum of coders_agree/total nrow coders_agree
#percent agreement for each code: (<code>_coder1 + <code>_coder2)/(sum of rows where these two cols aren't NA) -- for each coderpair
#OR create "<code>_agree" column for each code and then treat those as total (but only works if 2 coders coded each second)

#kappa -- not sure how to do this if not by coder pair
```
...
calc_reliability_coderpair <- function(coder_dfs) {
  ...
  for (i in 1:length(coder_dfs)) {
    ...
    #create empty dfs to percent agreements
    coderpair_summary_df <- data.frame(matrix(NA, nrow=4, ncol=length(codes))) #rows will be "perc_agree", "kappa", "baserate_c1", "baserate_c2"
    #columns will be code1, code2, etc 
    ...
    for (j in 1:length(codes)){
        if (codes[j] != "total") { #for all codes except total...
          ...
          #kappas
          coderpair_summary_df[2, j] = kappa2(cbind(coder_dfs[[i]][[coder1_col_name]], coder_dfs[[i]][[coder2_col_name]]))$value
          colnames(coderpair_summary_df)[j] <- paste0(codes[j])
          rownames(coderpair_summary_df)[2] <- paste0(str_c("kappa", sub("_code", "", coders[1]), sub("_code", "", coders[2]), sep="_"))
     
          #base rates for each coder and code
          coderpair_summary_df[3, j] = sum(coder_dfs[[i]][[coder1_col_name]], na.rm=T)/sum(!is.na(coder_dfs[[i]][[coder1_col_name]]))
          rownames(coderpair_summary_df)[3] <- paste0(str_c("baserate_", sub("_code", "", coders[1]), "_w", sub("_code", "", coders[2]), sep=""))
          coderpair_summary_df[4, j] = sum(coder_dfs[[i]][[coder2_col_name]], na.rm=T)/sum(!is.na(coder_dfs[[i]][[coder2_col_name]]))
          rownames(coderpair_summary_df)[4] <- paste0(str_c("baserate_", sub("_code", "", coders[2]), "_w", sub("_code", "", coders[1]),sep=""))
          }
        else { #do a different procedure for total
          #percent agreement
          overall_agree_col <- "coders_agree"
          ...
          #total kappa
          coderpair_summary_df[2 , j] <- kappa2(cbind(coder_dfs[[i]][[coders[1]]], coder_dfs[[i]][[coders[2]]]))$value #calculate total kappa, add new variable 
          colnames(coderpair_summary_df)[j] <- paste0(codes[j])
          rownames(coderpair_summary_df)[2] <- paste0(str_c("kappa", sub("_code", "", coders[1]), sub("_code", "", coders[2]), sep="_"))
        }
      }
