...
### Calculate base rates for each coder across all coderpairs and participants
```{r}
...
#create empty dfs to percent agreements
all_summary_df <- data.frame(matrix(NA, nrow=(length(all_coders)+2), ncol=length(codes))) #rows will be "perc_agree", "kappa", "baserate_c1", "baserate_c2"
#columns will be kappa_allcoders, percent_agree_allcoders, and base_rate for each coder
...
for (j in 1:length(codes)){
  ...
  all_summary_df[5, j] = mean(coderpairs_summary[grepl("kappa", coderpairs_summary$statistic), j+1])
  rownames(all_summary_df)[5] = "kappa_allcoders"
  
  #percent agreement as average of percent agreement across all coder pairs
  all_summary_df[6, j] = mean(coderpairs_summary[grepl("percent", coderpairs_summary$statistic), j+1])
  rownames(all_summary_df)[6] = "percent_agree_allcoders"
}
