...
create_kappa_baserates_dfs <- function(merged_dfs) {
  ...
  for (i in 1:length(merged_dfs)) {
    ...
    merged_dfs[[names(merged_dfs)[i]]] <- mutate(merged_dfs[[names(merged_dfs)[i]]],
      ...
      across(contains("_code") & !contains("agree"), ~ifelse(str_detect(., "dysphoric"), 1, 0), .names="neg_dys_{col}"), #similarly changed on 4/28
      neg_angry_consens = ifelse(str_detect(`Consensus Code`, "angry"), 1, 0), #similarly changed on 4/28
      across(contains("_code") & !contains("agree"), ~ifelse(str_detect(., "angry"), 1, 0), .names="neg_angry_{col}"), #similarly changed, 4/28
...
mega_df <- mutate(mega_df,
      ...
      across(ends_with("_code") & !contains("agree"), ~ifelse(grepl("dysphoric", .), 1, 0), .names="neg_dys_{sub('_code', '', col)}"),
      neg_angry_consens = ifelse(grepl("angry", `Consensus Code`), 1, 0),
      across(ends_with("_code") & !contains("agree"), ~ifelse(grepl("angry", .), 1, 0), .names="neg_angry_{sub('_code', '', col)}"),
      )
