...
Week 8 of Spring started piloting behavior certification codes. 
...
for (df_name in names(merged_dfs)) {
  ...
  merged_dfs[[df_name]] <- mutate(merged_dfs[[df_name]], coders_agree := case_when(
      ...
      grepl("pressures", get(coders[1]), ignore.case=TRUE) & grepl("pressures", get(coders[2]), ignore.case=TRUE) ~ 1,
      grepl("disengaged", get(coders[1]), ignore.case=TRUE) & grepl("disengaged", get(coders[2]), ignore.case=TRUE) ~ 1,
      grepl("requests", get(coders[1]), ignore.case=TRUE) & grepl("requests", get(coders[2]), ignore.case=TRUE) ~ 1,
      grepl("warm", get(coders[1]), ignore.case=TRUE) & grepl("warm", get(coders[2]), ignore.case=TRUE) ~ 1,
      grepl("auto", get(coders[1]), ignore.case=TRUE) & grepl("auto", get(coders[2]), ignore.case=TRUE) ~ 1,
      grepl("explanation", get(coders[1]), ignore.case=TRUE) & grepl("explanation", get(coders[2]), ignore.case=TRUE) ~ 1,
...
merged_dfs[[df_name]] <- mutate(merged_dfs[[df_name]], 
   ...
   across(ends_with("_code"), ~ifelse(grepl("pressures", .), 1, 0), .names="pressures_{sub('_code', '', col)}"),
   across(ends_with("_code"), ~ifelse(grepl("disengaged", .), 1, 0), .names="disengaged_{sub('_code', '', col)}"),
   across(ends_with("_code"), ~ifelse(grepl("requests", .), 1, 0), .names="requests_{sub('_code', '', col)}"),
   across(ends_with("_code"), ~ifelse(grepl("warm", .), 1, 0), .names="warm_{sub('_code', '', col)}"),
   across(ends_with("_code"), ~ifelse(grepl("auto", .), 1, 0), .names="auto_{sub('_code', '', col)}"),
   across(ends_with("_code"), ~ifelse(grepl("explanation", .), 1, 0), .names="explan_{sub('_code', '', col)}"),
...
calc_reliability_participant <- function(merged_dfs) {
  ...
  for (i in 1:length(merged_dfs)) {
    ...
    codes <- c("neutral", "pressures", "disengaged", "requests", "warm", "auto", "explanation", "uncodable", "total")
...
```
STOPPED HERE FOR EDITING FOR BEHAVIOR
