...
---
NOTE: finished editing for behavior, neeed to test
...
#ISSUE WHERE PAIR AFTER LONE FILE DOESN'T GET READ IN -- not sure why this happened once and then re-run it worked
...
calc_reliability_each_p_1row <- function(child_dfs) {
  ...
  # 7 = percent_agree, prescence agree, kappa, baserate coder 1, baserate coder 2, baserate_codable coder 1, baserate_codable coder 2 -- for each code 
  ...
  for (i in 1:length(child_dfs)) {
    ...
    for (j in 1:length(codes)){
        ...
        # sum of the base rate column for that coder, divided by sum of zeros in uncodable for that coder
          ...
          colnames(child_summary_df_1row)[(6 + (7*(j-1)))] <- paste0(str_c("baserate_codable", codes[j], "coder1", sep="_"))
          ...
          colnames(child_summary_df_1row)[(7 + (7*(j-1)))] <- paste0(str_c("baserate_codable", codes[j], "coder2", sep="_"))
        }
        else { #do a different procedure for total
          #percent agreement
          overall_agree_col <- "coders_agree"
