...
calc_reliability_each_p_1row <- function(child_dfs) {
  ...
  child_summary_df_1row <-  data.frame(matrix(NA, nrow=length(child_dfs), ncol=(((length(codes)-1)*5)+6))) #6 = total pc agree and kappa, seconds coded and coder for 1 and 2
  # 5 = percent_agree, prescence agree, kappa, baserate coder 1, baserate coder 2 -- for each code 
  ...
  for (i in 1:length(child_dfs)) {
    ...
    for (j in 1:length(codes)){
        if (codes[j] != "total") { #for all codes except total...
          coder1_col_name = str_c(codes[j], sub("_code", "", coders[1]), sep="_") #coder1 dummy code column name e.g., neutral_MA
          coder2_col_name = str_c(codes[j], sub("_code", "", coders[2]), sep="_") #coder2 dummy code column name neutral_CB
          #percent agreement (column numbers are 1, 6, 11, etc.)
          ...
          #presence agreement (column numbers are 2, 7, 12, etc.)
          ...
          #kappas (column numbers are 3, 8, 9, 13, etc)
          ...
          #percent agreement
          overall_agree_col <- "coders_agree"
          child_summary_df_1row[i, 41] = sum(child_dfs[[i]][[overall_agree_col]], na.rm=T)/sum(!is.na(child_dfs[[i]][[overall_agree_col]]))
          colnames(child_summary_df_1row)[41] <- paste0(str_c(codes[j], "_percent_agree", sep=""))
          rownames(child_summary_df_1row)[i] <- names(child_dfs[i])
          
          #total kappa
          child_summary_df_1row[i , 42] <- kappa2(cbind(child_dfs[[i]][[coders[1]]], child_dfs[[i]][[coders[2]]]))$value #calculate total kappa, add new variable 
          colnames(child_summary_df_1row)[42] <- paste0(str_c(codes[j], "_kappa", sep=""))
          rownames(child_summary_df_1row)[i] <- names(child_dfs[i])
        }
    }
    #add columns for number of seconds coded by each coder
    child_summary_df_1row[i, 43] = sum(!is.na(child_dfs[[i]][[coders[1]]]))
    colnames(child_summary_df_1row)[43] <- "seconds_coded_coder1"
    child_summary_df_1row[i, 44] = sum(!is.na(child_dfs[[i]][[coders[1]]]))
    colnames(child_summary_df_1row)[44] <- "seconds_coded_coder2" 
    
    #add columns for who is coder 1 and coder2
    child_summary_df_1row[i, 45] = paste0(sub("_code", "", coders[1]))
    colnames(child_summary_df_1row)[45] <- "coder1"
    child_summary_df_1row[i, 46] = paste0(sub("_code", "", coders[2]))
    colnames(child_summary_df_1row)[46] <- "coder2" 
  }
  #create 'coder' first column
