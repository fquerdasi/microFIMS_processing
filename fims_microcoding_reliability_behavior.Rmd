---
title: "fims_microcoding_reliability -- behavior"
...
output_path <- "../../FIMS/FIMS_micro_coding/MicroFIMS Core Team Training/microFIMS_reliability_data/behavior"
...
## Load data
Behavior reliability started rolling out in Fall 2023. 
...
for (i in 1:length(fl)) { #for each file in the list
  ...
  if ((gsub("fims_micro/", "", str_match(fl[[i]], "micro/(.*?)/fims")[2]) == str_match(fl[[i]], "behavior/(.*?)_[A-Za-z]")[2]) & !(fl[[i]] %in% empty)) {
    ...
    print(paste0("file ", str_match(fl[[i]], 'behavior/(.*)')[2], " and folder ", gsub("fims_micro/", "", str_match(fl[[i]], "micro/(.*?)/fims")[2]), " names do not match, not being read into R now -- please check"))
  }
}
...
# reliability_dfs = reliability_dfs[-which(sapply(names(reliability_dfs), is.na))]

for (df_name in names(reliability_dfs)) {
  reliability_dfs[[df_name]] <- dplyr::mutate(reliability_dfs[[df_name]], across(contains("_code"), ~str_to_lower(.), .names="{col}"))
}
```
...
```{r}
#filter out "" entries that got fed in  
...
```{r}
path <- "../../FIMS/FIMS_micro_coding/MicroFIMS Core Team Training/microFIMS_reliability_data/behavior/" #path to write the files
  ...
  for (p in 1:ncol(coder_pairs)){
    ...
    if(nrow(mega_df[!is.na(mega_df[,all_coders[coder_pairs[1, p]]]) & !is.na(mega_df[,all_coders[coder_pairs[2, p]]]),]) >0) {
      df <- mega_df[!is.na(mega_df[all_coders[coder_pairs[1, p]]]) & !is.na(mega_df[all_coders[coder_pairs[2, p]]]), ] %>% dplyr::select(
        ...
        relative_time,
        coders_agree,
        contains(sub("_code", "", all_coders[coder_pairs[1, p]]), ignore.case=FALSE),
        contains(sub("_code", "", all_coders[coder_pairs[2, p]]), ignore.case=FALSE)
      )
...
#create empty dfs for statistics
...
for (j in 1:length(codes)){
  for (k in 1:length(parent_coders)){
    if (codes[j] != "total") { #for all codes except total...
    #base rates for each coder and code with denominator being all seconds
...
colnames(all_summary_df)[9] <- paste0(codes[9])
colnames(parent_summary_df)[9] <- paste0(codes[9])
colnames(child_summary_df)[9] <- paste0(codes[9])
...
```
the empty rows will be filled in chunk 18 (not done w that tho)

# Create summary child and parent files with 1 row per participant
Child summary and parent summary
TEST OUT RUNNING CHUNK BELOW WITH CHANGES YOU FINISHED IMPLEMENTING
```{r}
...
calc_reliability_each_p_1row <- function(child_dfs) {
  ...
  child_summary_df_1row <-  data.frame(matrix(NA, nrow=length(child_dfs), ncol=(((length(codes)-1)*7)+6))) #6 = total pc agree and kappa, seconds coded and coder for 1 and 2
  # 7 = percent_agree, prescence agree, kappa, baserate coder 1, baserate coder 2, proportion coder 1, proportion coder 2 -- for each code 
  ...
  for (i in 1:length(child_dfs)) {
    ...
    for (j in 1:length(codes)){
        if (codes[j] != "total") { #for all codes except total...
          coder1_col_name = str_c(codes[j], sub("_code", "", coders[1]), sep="_") #coder1 dummy code column name e.g., neutral_MA
          coder2_col_name = str_c(codes[j], sub("_code", "", coders[2]), sep="_") #coder2 dummy code column name neutral_CB
          #percent agreement (column numbers are 1, 8, 15, etc.)
          child_summary_df_1row[i, (1+((j-1)*7))] = (sum(apply(child_dfs[[i]][,startsWith(colnames(child_dfs[[i]]), codes[j])]==1, 1, all)) + sum(apply(child_dfs[[i]][,startsWith(colnames(child_dfs[[i]]), codes[j])]==0, 1, all)))/sum(apply(!is.na(child_dfs[[i]][,startsWith(colnames(child_dfs[[i]]), codes[j])]), 1, all))
          colnames(child_summary_df_1row)[(1+((j-1)*7))] <- paste0(str_c(codes[j], "_percent_agree", sep="")) #
          rownames(child_summary_df_1row)[i] <- names(child_dfs[i]) #
            
          #presence agreement (column numbers are 2, 9, 16, etc.)
          child_summary_df_1row[i, (2 + (7*(j-1)))] = (sum(apply(child_dfs[[i]][,startsWith(colnames(child_dfs[[i]]), codes[j])]==1, 1, all)))/sum(child_dfs[[i]][, coder1_col_name] == 1 | child_dfs[[i]][, coder2_col_name] == 1, na.rm=T) #total number of times both cols are 1 divided by nrows with any instances of that code
          colnames(child_summary_df_1row)[(2 + (7*(j-1)))] <- paste0(str_c(codes[j], "_prescence_agree", sep="")) #
          rownames(child_summary_df_1row)[i] <- names(child_dfs[i]) #
          
          #kappas (column numbers are 3, 10, 17, etc)
          child_summary_df_1row[i, (3 + (7*(j-1)))] = kappa2(cbind(child_dfs[[i]][[coder1_col_name]], child_dfs[[i]][[coder2_col_name]]))$value
          colnames(child_summary_df_1row)[(3 + (7*(j-1)))] <- paste0(str_c(codes[j], "_kappa", sep=""))
          rownames(child_summary_df_1row)[i] <- names(child_dfs[i]) #
          
          #base rates for each coder and code (column numbers are 4, 5, 11, 12, etc.)
          child_summary_df_1row[i, (4 + (7*(j-1)))] = sum(child_dfs[[i]][[coder1_col_name]], na.rm=T)/sum(!is.na(child_dfs[[i]][[coder1_col_name]]))
          colnames(child_summary_df_1row)[(4 + (7*(j-1)))] <- paste0(str_c("baserate", codes[j], "coder1", sep="_"))
          child_summary_df_1row[i, (5 + (7*(j-1)))] = sum(child_dfs[[i]][[coder2_col_name]], na.rm=T)/sum(!is.na(child_dfs[[i]][[coder2_col_name]]))
          colnames(child_summary_df_1row)[(5 + (7*(j-1)))] <- paste0(str_c("baserate", codes[j], "coder2", sep="_"))
        ...
        # sum of the base rate column for that coder, divided by sum of zeros in uncodable for that coder
          child_summary_df_1row[i, (6 + (7*(j-1)))] = sum(child_dfs[[i]][[coder1_col_name]], na.rm=T)/sum(child_dfs[[i]][[str_c("uncodable", sub("_code", "", coders[1]), sep="_")]]==0)
          colnames(child_summary_df_1row)[(6 + (7*(j-1)))] <- paste0(str_c("proportion", codes[j], "coder1", sep="_"))
          child_summary_df_1row[i, (7 + (7*(j-1)))] = sum(child_dfs[[i]][[coder2_col_name]], na.rm=T)/sum(child_dfs[[i]][[str_c("uncodable", sub("_code", "", coders[2]), sep="_")]]==0)
          colnames(child_summary_df_1row)[(7 + (7*(j-1)))] <- paste0(str_c("proportion", codes[j], "coder2", sep="_"))
        }
        else { #do a different procedure for total
          #percent agreement
          overall_agree_col <- "coders_agree"
          child_summary_df_1row[i, 57] = sum(child_dfs[[i]][[overall_agree_col]], na.rm=T)/sum(!is.na(child_dfs[[i]][[overall_agree_col]]))
          colnames(child_summary_df_1row)[57] <- paste0(str_c(codes[j], "_percent_agree", sep=""))
          rownames(child_summary_df_1row)[i] <- names(child_dfs[i])
          
          #total kappa
          child_summary_df_1row[i , 58] <- kappa2(cbind(child_dfs[[i]][[coders[1]]], child_dfs[[i]][[coders[2]]]))$value #calculate total kappa, add new variable 
          colnames(child_summary_df_1row)[58] <- paste0(str_c(codes[j], "_kappa", sep=""))
          rownames(child_summary_df_1row)[i] <- names(child_dfs[i])
        }
    }
    #add columns for number of seconds coded by each coder
    child_summary_df_1row[i, 59] = sum(!is.na(child_dfs[[i]][[coders[1]]]))
    colnames(child_summary_df_1row)[59] <- "seconds_coded_coder1"
    child_summary_df_1row[i, 60] = sum(!is.na(child_dfs[[i]][[coders[1]]]))
    colnames(child_summary_df_1row)[60] <- "seconds_coded_coder2" 
    
    #add columns for who is coder 1 and coder2
    child_summary_df_1row[i, 61] = paste0(sub("_code", "", coders[1]))
    colnames(child_summary_df_1row)[61] <- "coder1"
    child_summary_df_1row[i, 62] = paste0(sub("_code", "", coders[2]))
    colnames(child_summary_df_1row)[62] <- "coder2" 
  }
  #create 'coder' first column
...
# add _parent and _child to all variables in these two datasets (to make merging later easier)
child_summary_df_1row_final <- child_summary_df_1row_final %>% dplyr::rename_with(., ~paste0(.x, "_child", sep = ""), !participant)
parent_summary_df_1row_final <- parent_summary_df_1row_final %>% dplyr::rename_with(., ~paste0(.x, "_parent", sep = ""), !participant)
```
...
NOTE: for now, if any percent agree, etc. are NA for a coder pair, remove that coder pair from the mean calc (na.rm=TRUE) instead of having the mean be NA
```{r}
#calculate reliability
for (j in 1:length(codes)){
  #kappa_allcoders for each code can be calculated as the average kappa across all coder pairs
  all_summary_df[length(all_coders)+1, j] = mean(coderpairs_summary[grepl("kappa", coderpairs_summary$statistic), j+1], na.rm=TRUE)
  ...
  all_summary_df[length(all_coders)+2, j] = mean(coderpairs_summary[grepl("percent", coderpairs_summary$statistic), j+1], na.rm=TRUE)
  ...
  all_summary_df[length(all_coders)+3, j] = mean(coderpairs_summary[grepl("presence", coderpairs_summary$statistic), j+1], na.rm=TRUE)
