...
Column that identifies randomly assigned master coder for videos that don't have tiebreakers and were not used in certification
### Tiebreaker codes
```{r}
...
for (df_name in names(tiebreaker_code_dfs)) {
  ...
  tiebreaker_code_dfs[[df_name]] <- dplyr::rename_with(tiebreaker_code_dfs[[df_name]], .fn = ~gsub("final", "master", .), .cols = contains("final"))
}
```
## Certification codes
```{r}
...
certification_codes[[df_name]] <- mutate(certification_codes[[df_name]], 
   across(`Consensus Code`, ~ifelse(grepl("neutral", .), 1, 0), .names="neutral_master"),
   across(`Consensus Code`, ~ifelse(grepl("active", .), 1, 0), .names="active_master"),
   across(`Consensus Code`, ~ifelse(grepl("positive", .), 1, 0), .names="positive_master"),
   across(`Consensus Code`, ~ifelse(grepl("withdraw", .), 1, 0), .names="withdraw_master"),
   across(`Consensus Code`, ~ifelse(grepl("off", .), 1, 0), .names="offtask_master"),
   across(`Consensus Code`, ~ifelse(grepl("uncodable", .), 1, 0), .names="uncodable_master"),
   across(`Consensus Code`, ~ifelse(grepl("auto", .), 1, 0), .names="nonauto_master"),
   across(`Consensus Code`, ~ifelse(grepl("reject", .), 1, 0), .names="reject_master")
) 
}

# get rid of extra columns and create secondary_id column
for (df_name in names(certification_codes)) {
  # rename consensus code column to be 'master_codes'
  certification_codes[[df_name]] <- dplyr::rename(certification_codes[[df_name]], master_codes = `Consensus Code`)
  #create secondary_id column
  ...
  certification_codes[[df_name]] <- dplyr::select(certification_codes[[df_name]], secondary_id, contains("master"), relative_time)
  
}
```
## Other participants
```{r}
# load the file with assignment for coder1 or coder2 to be master (validity dataset)
coder_assignment <- read_csv("data/microfims_validity_final_data.csv") %>% dplyr::select(secondary_id, master_coder)
...
# rename the appropriate columns to be 'master' coder
# match secondary_id (minus _child or _parent) to the key doc to determine which end string to change to 'master'
for (df_name in names(general_code_dfs)) {
  df_id <- gsub("_child|_parent", "", general_code_dfs[[df_name]]$secondary_id[1])
  master_column <- as.character(coder_assignment[coder_assignment$secondary_id == df_id, "master_coder"])
  general_code_dfs[[df_name]] <- rename_with(general_code_dfs[[df_name]],  ~gsub("_[^_]+$", "_master", .), .cols = ends_with(master_column))
  general_code_dfs[[df_name]] <- rename_with(general_code_dfs[[df_name]],  ~gsub("coder[12]_", "master_", .), .cols = starts_with(master_column))
}

# drop the non-master codes
for (df_name in names(general_code_dfs)) {
  general_code_dfs[[df_name]] <- dplyr::select(general_code_dfs[[df_name]], secondary_id, contains("master"), relative_time)
  general_code_dfs[[df_name]] <- dplyr::rename(general_code_dfs[[df_name]], master_codes = master_code)
}
...
for (df_name in names(all_code_dfs)) {
  all_code_dfs[[df_name]] <- dplyr::mutate(all_code_dfs[[df_name]], across(ends_with("master") & !contains("uncodable"), ~ifelse(uncodable_master==1, NA, .)))
}

# remove uncodable column
for (df_name in names(all_code_dfs)) {
  all_code_dfs[[df_name]] <- dplyr::select(all_code_dfs[[df_name]], secondary_id, relative_time, contains("master") & !contains("uncodable"))
}
